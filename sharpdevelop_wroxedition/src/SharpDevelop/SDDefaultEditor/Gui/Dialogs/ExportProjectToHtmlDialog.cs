...
namespace ICSharpCode.SharpDevelop.Gui.Dialogs
{
	public class ExportProjectToHtmlDialog : Form
	{
		...
		{
			if (filename != null) {
				return fileUtilityService.GetImageIndexForFile(filename);
			}
			return -1;
		}
				...
				foreach (TextWord word in line.Words) {
					switch (word.Type) {
						...
						case TextWordType.Tab:
							for (int k = 0; k < document.Properties.GetProperty("TabIndent", 4); ++k) {
								curFileStream.Write("&nbsp;");
							}
						break;
			...
			foreach (DictionaryEntry entry in table) {
				if (entry.Value is Hashtable) {
					...
					if (bitmapTable[idx] == null) {
						...
						Bitmap bmp = (Bitmap)fileUtilityService.ImageList.Images[idx];
		...
		void ExportProject(object sender, EventArgs e)
		{
			if (!Directory.Exists(pathTextBox.Text)) {
				MessageBox.Show("Directroy doesn't exists", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return;
			}
