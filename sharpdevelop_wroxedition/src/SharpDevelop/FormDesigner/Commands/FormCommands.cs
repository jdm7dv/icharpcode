...
namespace ICSharpCode.SharpDevelop.FormEditor.Commands
{
	...
	public abstract class AbstractFormDesignerCommand : AbstractMenuCommand
	{
		...
		public override void Run()
		{
			...
			FormDesignerDisplayBindingBase formDesigner = window.ViewContent as FormDesignerDisplayBindingBase;
	...
	{
		public Crownwood.Magic.Menus.MenuCommand[] BuildSubmenu(object owner)
		{
			...
			if (items.Count > 0) {
				items.Add(new Crownwood.Magic.Menus.MenuCommand("-"));
			}
			
			return (Crownwood.Magic.Menus.MenuCommand[])items.ToArray(typeof(Crownwood.Magic.Menus.MenuCommand));
		}
		
		class ContextMenuCommand : Crownwood.Magic.Menus.MenuCommand
		{
			DesignerVerb verb;
			
			public ContextMenuCommand(DesignerVerb verb) : base(verb.ToString())
			{
				this.Text = verb.Text;
