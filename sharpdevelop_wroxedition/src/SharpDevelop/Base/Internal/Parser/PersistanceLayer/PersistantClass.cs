...
{
	public sealed class PersistantClass : IClass
	{
		int                  classProxyIndex;
		...
		public PersistantClass(BinaryReader reader, ClassProxyCollection classProxyCollection)
		{
			...
			for (uint i = 0; i < count; ++i) {
				innerClasses.Add(new PersistantClass(reader, classProxyCollection));
			}
			
			count = reader.ReadUInt32();
			for (uint i = 0; i < count; ++i) {
				fields.Add(new PersistantField(reader, classProxyCollection));
			}
			
			count = reader.ReadUInt32();
			for (uint i = 0; i < count; ++i) {
				properties.Add(new PersistantProperty(reader, classProxyCollection));
			}
			
			count = reader.ReadUInt32();
			for (uint i = 0; i < count; ++i) {
				IMethod m = new PersistantMethod(reader, classProxyCollection);
				methods.Add(m);
			}
			
			count = reader.ReadUInt32();
			for (uint i = 0; i < count; ++i) {
				events.Add(new PersistantEvent(reader, classProxyCollection));
			}
			
			count = reader.ReadUInt32();
			for (uint i = 0; i < count; ++i) {
				indexer.Add(new PersistantIndexer(reader, classProxyCollection));
			}
		}
		...
		public void WriteTo(BinaryWriter writer)
		{
			...
			foreach (PersistantClass innerClass in innerClasses) {
				innerClass.WriteTo(writer);
			}
			
			writer.Write((uint)fields.Count);
			foreach (PersistantField field in fields) {
				field.WriteTo(writer);
			}
			
			writer.Write((uint)properties.Count);
			foreach (PersistantProperty property in properties) {
				property.WriteTo(writer);
			}
			
			writer.Write((uint)methods.Count);
			foreach (PersistantMethod method in methods) {
				method.WriteTo(writer);
			}
			
			writer.Write((uint)events.Count);
			foreach (PersistantEvent e in events) {
				e.WriteTo(writer);
			}
			
			writer.Write((uint)indexer.Count);
			foreach (PersistantIndexer ind in indexer) {
				ind.WriteTo(writer);
			}
		}
		
		public PersistantClass(ClassProxyCollection classProxyCollection, IClass c)
		{
			this.classProxyCollection = classProxyCollection;
			...
			foreach (IClass innerClass in c.InnerClasses) {
				innerClasses.Add(new PersistantClass(classProxyCollection, innerClass));
			}
			
			foreach (IField field in c.Fields) {
				fields.Add(new PersistantField(classProxyCollection, field));
			}
			
			foreach (IProperty property in c.Properties) {
				properties.Add(new PersistantProperty(classProxyCollection, property));
			}
			
			foreach (IMethod method in c.Methods) {
				methods.Add(new PersistantMethod(classProxyCollection, method));
			}
			
			foreach (IEvent e in c.Events) {
				events.Add(new PersistantEvent(classProxyCollection, e));
			}
			
			foreach (IIndexer ind in c.Indexer) {
				indexer.Add(new PersistantIndexer(classProxyCollection, ind));
			}
		}
