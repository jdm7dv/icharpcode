		...
		public string Title {
			...
			set {
				...
				if (fileName != null) {
					...
					tabPage.ImageIndex = fileUtilityService.GetImageIndexForFile(fileName);
				}
				OnTitleChanged(null);
			}
		}
...
// KSL End
		...
		public SdiWorkspaceWindow(IViewContent content, Crownwood.Magic.Controls.TabControl tabControl)
		{
			...
			tabPage.Tag = this;
			FileUtilityService fileUtilityService = (FileUtilityService)ServiceManager.Services.GetService(typeof(FileUtilityService));
			tabPage.ImageList = fileUtilityService.ImageList;
				...
				switch (dr) {
					case DialogResult.Yes:
						if (content.ContentName == null) {
							...
							FileUtilityService fileUtilityService = (FileUtilityService)ServiceManager.Services.GetService(typeof(FileUtilityService));
							fileUtilityService.ObservedSave(new FileOperationDelegate(ViewContent.SaveFile), ViewContent.ContentName , FileErrorPolicy.ProvideAlternative);
						}
						break;
