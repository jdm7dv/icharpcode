...
namespace ICSharpCode.SharpDevelop.Gui.Pads.ProjectBrowser
{
	...
	public class ProjectBrowserView : TreeView, IPadContent, IMementoCapable
	{
		...
		public Bitmap Icon {
			get {
				return resourceService.GetBitmap("Icons.16x16.CombineIcon");
			}
		}
		...
		void OpenCombine(object sender, CombineEventArgs e)
		{
			DisposeProjectNodes();
			Nodes.Clear();
			TreeNode treeNode = BuildCombineTreeNode(e.Combine);
			Nodes.Add(treeNode);
			e.Combine.StartupPropertyChanged += new EventHandler(StartupPropertyChanged);
		...
		void StartupPropertyChanged(object sender, EventArgs e)
		{
			...
			Console.WriteLine("STARTUP PRoPERTY CHANGED");
		...
		{
			IXmlConvertable memento = CreateMemento();
			Nodes.Clear();
			System.GC.Collect();
			Nodes.Add(BuildCombineTreeNode(WorkbenchSingleton.Workbench.ProjectManager.CurrentOpenCombine));
			SetMemento(memento);
		}*/
		...
		public static AbstractBrowserNode BuildCombineTreeNode(Combine combine)
		{
			...
			foreach (CombineEntry entry in combine.Entries) {
				if (entry.Entry is IProject) {
					combineNode.Nodes.Add(BuildProjectTreeNode((IProject)entry.Entry));
				} else {
					combineNode.Nodes.Add(BuildCombineTreeNode((Combine)entry.Entry));
				}
			}
		...
		{
			if (e.Button == MouseButtons.Right && SelectedNode != null && SelectedNode is AbstractBrowserNode) {
		    	AbstractBrowserNode selectedBrowserNode = (AbstractBrowserNode)SelectedNode;
		...
		static ImageList projectBrowserImageList  = new ImageList();
		...
		static ProjectBrowserNode GetRootProjectNode(AbstractBrowserNode node)
		{
			...
			return GetRootProjectNode((AbstractBrowserNode)node.Parent);
		}
