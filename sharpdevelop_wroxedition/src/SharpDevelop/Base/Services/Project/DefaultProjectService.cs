...
namespace ICSharpCode.SharpDevelop.Services
{
	public class DefaultProjectService : AbstractService, IProjectService
	{
		...
		public void OpenCombine(string filename)
		{
			...
			if (Path.GetExtension(filename).ToUpper() == ".PRJX") {
				...
				if (File.Exists(validcombine)) {
					...
					openCombine = new Combine();
					IProject project = (IProject)openCombine.AddEntry(filename);
					if (project == null) {
						CloseCombine();
						return;
					}
					openCombine.Name = project.Name;
					openCombine.SaveCombine(validcombine);
					openCombineFileName = validcombine;
					LoadCombine(validcombine);
				}
			} else {
		...
		void LoadCombine(string filename)
		{
			...
			openCombine = new Combine();
			openCombine.LoadCombine(openCombineFileName);
		...
		void SaveCombinePreferences(Combine combine, string combinefilename)
		{
			...
			properties.SetProperty("ActiveWindow", WorkbenchSingleton.Workbench.ActiveWorkbenchWindow == null ? "" : WorkbenchSingleton.Workbench.ActiveWorkbenchWindow.ViewContent.ContentName);
		...
		public void RenameProject(IProject project, string newName)
		{
			// check if there is already a project with name newName
			ArrayList allProjects = Combine.GetAllProjects(openCombine);
			int number = 0;
			
			foreach (ProjectCombineEntry projectEntry in allProjects) {
				if (projectEntry.Project.Name == newName) {
					number++;
					if (number >= 2) {
						throw new ApplicationException("duplicate name " + newName);
					}
				}
			}
			string oldName = project.Name;
			project.Name = newName;
			OnRenameProject(new ProjectRenameEventArgs(project, oldName, newName));
		}
