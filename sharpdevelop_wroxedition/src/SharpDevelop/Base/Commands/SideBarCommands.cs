...
namespace ICSharpCode.SharpDevelop.Commands
{
	...
	public class SideBarDeleteTabItem: AbstractMenuCommand
	{
		...
		{
			SharpDevelopSideBar sideBar = (SharpDevelopSideBar)Owner;
			AxSideTabItem item = sideBar.ActiveTab.GetItemAt(sideBar.ItemMousePosition);
			ResourceService resourceService = (ResourceService)ServiceManager.Services.GetService(typeof(ResourceService));
			StringParserService stringParserService = (StringParserService)ServiceManager.Services.GetService(typeof(StringParserService));
			
			if (item != null &&
			    MessageBox.Show(stringParserService.Parse(resourceService.GetString("SideBarComponent.ContextMenu.DeleteTabItemQuestion"), new string[,] { {"TabItem", item.Name}}),
