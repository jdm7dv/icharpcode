...
namespace ICSharpCode.SharpDevelop.Commands
{
	public class CreateNewProject : AbstractMenuCommand
	{
		public override void Run()
		{
			...
			using (NewProjectDialog npdlg = new NewProjectDialog()) {
				npdlg.Owner = (Form)WorkbenchSingleton.Workbench;
				if (npdlg.ShowDialog() == DialogResult.OK) {
					projectService.OpenCombine(npdlg.NewCombineLocation);
				}
			}
		}
	}
	...
	public class ReloadFile : AbstractMenuCommand
	{
		public override void Run()
		{
			...
			if (window != null && window.ViewContent.ContentName != null) {
				...
				window.ViewContent.LoadFile(window.ViewContent.ContentName);
			...
			if (window != null) {
				...
				using (SaveFileDialog fdiag = new SaveFileDialog()) {
					...
					if (fdiag.ShowDialog() == DialogResult.OK) {
						...
						if (fileUtilityService.ObservedSave(new NamedFileOperationDelegate(window.ViewContent.SaveFile), fileName) == FileOperationResult.OK) {
				...
				if (fdiag.ShowDialog() == DialogResult.OK) {
					switch (Path.GetExtension(fdiag.FileName).ToUpper()) {
						...
						case ".PRJX":
							...
							break;
						default:
							MessageBox.Show("Can't open file " + fdiag.FileName + "as project", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
							break;
					}
				}
			}
		...
		public override void Run()
		{
			...
			if (window != null) {
				if (window.ViewContent is IPrintable) {
					...
					MessageBox.Show("Can't print this window content", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
				}
			}
		}
	}
