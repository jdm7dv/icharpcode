...
namespace ICSharpCode.TextEditor.Document
{
	public class DefaultHighlightingStrategy : IHighlightingStrategy
	{
		...
		HighlightColor caretColor       = null; 
		...
		HighlightColor bookmarkColor    = null;
		...
		{
			defaultColor    = new HighlightBackground(Color.Black, Color.White, false, false);
			lineNumberColor = new HighlightColor(Color.Gray, false, false);
			eolMarkerColor  = new HighlightColor(Color.DarkCyan, false, false);
			digitColor      = new HighlightColor(Color.Black, false, false);
			invalidLineColor= new HighlightColor(Color.Red, false, false);
			caretColor      = new HighlightColor(Color.Blue, false, false);
			selectionColor  = new HighlightColor(Color.White, Color.LightBlue, false, false);
			vRulerColor     = new HighlightColor(Color.Blue, false, false);
			spaceMarkerColor= new HighlightColor(Color.DarkCyan, false, false);
			tabMarkerColor  = new HighlightColor(Color.DarkCyan, false, false);
			bookmarkColor   = new HighlightColor(Color.FromArgb(0xFF ,0x40, 0x80, 0x80), false, false);
		...
		void ResolveRuleSetReferences() 
		{
			foreach (HighlightRuleSet ruleSet in Rules) {
				...
				foreach (Span aSpan in ruleSet.Spans) {
					if (aSpan.Rule != null) {
						...
						if (!found) {
							MessageBox.Show("The RuleSet " + aSpan.Rule + " could not be found in mode definition " + this.Name,
							                "SharpDevelop", MessageBoxButtons.OK, MessageBoxIcon.Warning);
							aSpan.RuleSet = null;
						}
					} else {
						aSpan.RuleSet = null;
			...
			if (defaultRuleSet == null) {
				MessageBox.Show("No default RuleSet is defined for mode definition " + this.Name,
				                "SharpDevelop", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			...
			foreach (HighlightRuleSet ruleSet in Rules) {
				if (ruleSet.Reference != null) {
					...
					if (highlighter != null) {
						...
						MessageBox.Show("The mode defintion " + ruleSet.Reference + " which is refered from the " + this.Name + " mode definition could not be found",
						                "SharpDevelop", MessageBoxButtons.OK, MessageBoxIcon.Warning);
		...
		public HighlightColor GetColor(IDocumentAggregator document, LineSegment currentSegment, int currentOffset, int currentLength)
		{
			return GetColor(defaultRuleSet, document, currentSegment, currentOffset, currentLength);
		}

		HighlightColor GetColor(HighlightRuleSet ruleSet, IDocumentAggregator document, LineSegment currentSegment, int currentOffset, int currentLength)
		{
			if (ruleSet != null) {
				if (ruleSet.Reference != null) {
		...
		public void MarkTokens(IDocumentAggregator document)
		{
			if (Rules.Count == 0) {
				return;
			}
			...
			document.UpdateDocumentRequested = true;
		}
		
		bool MarkTokensInLine(IDocumentAggregator document, int lineNumber, ref bool spanChanged)
		{
		...
		public void MarkTokens(IDocumentAggregator document, LineSegmentCollection inputLines)
		{
			if (Rules.Count == 0) {
				return;
			}
			...
			if (spanChanged) {
				...
				document.UpdateCaretLineRequested = true;
			}
		}
		...
		ArrayList ParseLine(IDocumentAggregator document)
		{
			...
			for (int i = 0; i < currentLine.Length; ++i) {
				...
				switch (ch) {
					case '\n':
					case '\r':
						PushCurWord(document, markNext, words);
						++currentOffset;
						break;
					case ' ':
						PushCurWord(document, markNext, words);
		...
		/// correct color.
		/// </summary>
		void PushCurWord(IDocumentAggregator document, HighlightColor markNext, ArrayList words)
		{
